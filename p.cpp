...
class Arboles {
    ...
    public :
        ...
        void insertar_alf( aNodo*& nodo , Director* director, aNodo* curr_1);
...
void Arboles::insertar_alf( aNodo*& nodo , Director* director, aNodo* &curr) { // Inserta los nodos dentro del  arbol 1 alfabeticamente en in-orden
    int c = 0;
    if (nodo==nullptr && c == 0){
    ...
    if ( director -> get_nombre() < nodo -> val -> get_nombre() ){
        cout << "IZQUIERDA" << endl;
        insertar_alf ( nodo, director, curr -> izq);
    }
    if ( director -> get_nombre() > curr -> val -> get_nombre() ){
        cout << "DERECHA" << endl;
        insertar_alf ( nodo , director , nodo -> der );
...
int main() {
    ...
    for (int i = 0; i < cant_peliculas; i++) {
        ...
        pelicula->director = linea.substr(pos1 + 1  , pos2 - pos1 - 1); //obtiene el nombre del directo
